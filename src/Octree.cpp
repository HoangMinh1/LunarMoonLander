...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
    ...
    for(int i = 0; i < boxList.size(); i++) {
        ...
        if(count > 0) {
            ...
            numLeaf++;
            //            if child is not a leaf node (contains more than point)
            //               recursively call subdivide(child)
            if(count > 1) {
                level++;
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
    ...
    if(node.box.intersect(ray, t0, t1)) {
        if(node.points.size() == 1) { //intersect at one point
            nodeRtn = node;
            return true;
        }
        for(int i = 0; i < node.children.size(); i++) {
            intersects = intersects || intersect(ray, *(node.children[i]), nodeRtn);
        }
    }
	return intersects;
}
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
    ...
    if(node.box.overlap(box)) {
        if(node.points.size() == 1) { 
            ...
            for(int i = 0; i < node.children.size(); i++) {
                intersects = intersects || intersect(box, *(node.children[i]), boxListRtn);
            }
        }
    }
	return intersects;
