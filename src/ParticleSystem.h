...
class ThrustForce: public ParticleForce {
    float magnitude = 1.0;
    ofVec3f direction = ofVec3f(0, 0, 0);
public:
    void set(float mag, ofVec3f dir) {
        magnitude = mag;
        direction = dir;
    }
    ThrustForce(float mag, ofVec3f dir);
    ThrustForce() {}
    void updateForce(Particle *);
};

class ImpulseForce: public ParticleForce {  // force for collision
    ofVec3f force;
    ofVec3f velocity;
    ofVec3f norm;
    float e = 0.2;
public:
    ImpulseForce(ofVec3f vec, ofVec3f nor) {
        velocity = vec;
        norm = nor;
    }
    ImpulseForce();
